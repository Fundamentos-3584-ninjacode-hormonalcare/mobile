...
class _AppointmentFormState extends State<AppointmentForm> {
  ...
  final MedicalAppointmentRepository repository =
      MedicalAppointmentRepository(MedicalAppointmentApi());
  ...
  Future<bool> _isTimeSlotAvailable(String startTime, String endTime) async {
    ...
    final newStart = DateTime.parse(
        "${_selectedDate!.toIso8601String().split('T')[0]} $startTime:00");
    final newEnd = DateTime.parse(
        "${_selectedDate!.toIso8601String().split('T')[0]} $endTime:00");

    for (var appointment in existingAppointments) {
      final existingStart = DateTime.parse(
          "${appointment['eventDate']} ${appointment['startTime']}:00");
      final existingEnd = DateTime.parse(
          "${appointment['eventDate']} ${appointment['endTime']}:00");
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (!await _isTimeSlotAvailable(startTime, endTime)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('The selected time slot is not available.')));
        return;
      }
      ...
      final appointmentData = {
        ...
        'description': JitsiMeetingLinkGenerator.generateMeetingLink(
            roomPrefix: _titleController.text),
        'doctorId': userId,
      ...
      final success =
          await repository.createMedicalAppointment(appointmentData);
      if (success) {
        Navigator.pop(context);
        _confettiController.play();
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Medical appointment created successfully!')));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Failed to create medical appointment.')));
      }
    }
  }
      ...
      child: ConstrainedBox(
        constraints: BoxConstraints(
          maxHeight: MediaQuery.of(context).size.height *
              0.6, // Ajusta el valor según sea necesario
        ),
        child: Form(
          key: _formKey,
          child: Padding(
                ...
                TextFormField(
                  ...
                  validator: (value) {
                    ...
                    if (_selectedDate != null) {
                      final selectedDateInLima =
                          tz.TZDateTime.from(_selectedDate!, limaTimeZone);
                      final nowInLima = tz.TZDateTime.now(limaTimeZone);
                      if (selectedDateInLima.isBefore(
                          nowInLima.subtract(const Duration(days: 1)))) {
                      ...
                      child: TextFormField(
                        ...
                        validator: (value) {
                          ...
                          if (_fromTimeController.text.isNotEmpty &&
                              value != null) {
                            final fromTime = _fromTimeController.text
                                .split(':')
                                .map(int.parse)
                                .toList();
                            final toTime =
                            ...
                            final from =
                                DateTime(0, 0, 0, fromTime[0], fromTime[1]);
